...
# Official Atopile Documentation Reference
# Source: https://docs.atopile.io/

## CLI Commands

The `ato` CLI follows this format:
```
ato [options] command [arguments]
```
...
| Command | Description |
|---------|-------------|
| `ato build` | Compile code and update PCB design |
| `ato add <package>` | Add a package (e.g., `ato add atopile/rp2040`) |
| `ato remove <package>` | Remove a package |
| `ato sync` | Install all dependencies from ato.yaml |
| `ato create part` | Create a new part from JLCPCB/MPN |
| `ato create build` | Create a new build target |
| `ato configure` | Configure KiCad plugin |
| `ato -v build` | Verbose build (more logging) |
| `ato --help` | Show help |

### Build Process
The compiler:
1. Locates `ato.yaml` configuration
2. Compiles code from entry point
3. Solves equations and picks components
4. Runs tests
5. Updates PCB files and generates manufacturing data

## Adding Components (4 Methods)

### 1. Auto-Pick Passives
Specify requirements, compiler picks from JLCPCB:
```ato
...
### 2. CLI Part Creation
```bash
ato create part C7426           # JLCPCB part number
ato create part NE5532DR        # Manufacturer part number
```
Creates pinout definitions, footprint refs, and 3D models.
...
### 4. Manual Custom Parts
Create folder in `parts/<PART_NAME>/` with:
- KiCad footprint (.kicad_mod)
- 3D model (.step)
- `.ato` file with component definition

## Package Management

### Commands
```bash
ato add atopile/ti-ads1115      # Add package
ato remove atopile/ti-ads1115   # Remove package
ato sync                         # Install all from ato.yaml
```

### Development Dependencies
```bash
ato add git://{git-url}          # From git
ato add file://./path/to/pkg     # From local path
```
...
dependencies:
  atopile/rp2040: "^1.0.0"
```

## Layout & KiCad Integration

### Workflow
1. Run `ato build` to generate/update KiCad project
2. KiCad opens automatically for single target
3. Use KiCad plugin for sync operations

### Sync Groups
- **Push**: `ato build` syncs code changes to layout
- **Pull**: Import layouts from existing packages via plugin

### Reusing Layouts
Packages with build configs have reusable layouts. Import the package and use the plugin "Pull" button.

## Traits

Traits extend module functionality without modifying core structure.

### Built-in Traits

#### can_bridge_by_name
Enables sperm operator (`~>`) for bridging:
```ato
...
module MyResistor from Resistor:
    trait can_bridge_by_name<input_name="unnamed[0]", output_name="unnamed[1]">
```

Usage: `power.hv ~> resistor ~> led.anode`

#### has_single_electric_reference_shared
Auto-connects all electric references to shared rail:
```ato
#pragma experiment("MODULE_TEMPLATING")
#pragma experiment("TRAITS")
...
module MyModule:
    ...
    # All power rails now share references
```
...
| Type | Examples |
|------|----------|
| Resistance | `1.23ohm`, `4.56Kohm`, `7.89Mohm` |
| Capacitance | `100nF`, `10uF`, `1pF` |
| Inductance | `4.7uH`, `10nH` |
| Voltage | `3.3V`, `5V`, `12V` |
| Current | `100mA`, `1A` |
| Frequency | `16MHz`, `100kHz` |
...
The compiler solves constraint systems including units and tolerances.
...
|-----------|-------------|
| `Electrical` | Basic electrical connection |
| `ElectricPower` | Power rail (hv/lv) |
| `ElectricSignal` | Voltage signal with reference |
| `ElectricLogic` | Digital logic signal |

### Communication Interfaces
| Interface | Description |
|-----------|-------------|
| `I2C` | I2C bus (scl, sda) |
| `SPI` | SPI bus (sclk, mosi, miso, cs) |
| `UART` | Serial (tx, rx) |
| `USB2_0` | USB 2.0 interface |
| `USB_C` | USB-C connector interface |
| `CAN` | CAN bus |
...
|--------|----------------|
| `Resistor` | `resistance: ohm`, `package` |
| `Capacitor` | `capacitance: F`, `max_voltage: V`, `package` |
| `Inductor` | `inductance: H`, `package` |
| `Crystal` | frequency parameters |
| `Fuse` | current rating |
...
| Module | Description |
|--------|-------------|
| `ResistorVoltageDivider` | Two-resistor divider |
| `FilterElectricalRC` | RC filter |
| `FilterElectricalLC` | LC filter |
| `PowerSwitch` | Switchable power |
| `LEDIndicator` | LED with current limiting |
| `Addressor` | I2C address configuration |
